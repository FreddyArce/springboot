...
let editarEstudiante = async function(id) {
    let response = await axios.get(`/estudiantes/${id}`);
...
let registrarEstudiante = async function() {
    let estudiante = {
        id: (!hidId || hidId.value == '') ? null : hidId.value,
    ...
    window.location.href = '/estudiantes/list';
}
...
let mostrarEstudiantes = async function() {
    ...
    for (let estudiante of estudiantes) {
        ...
        btnEditar.addEventListener('click', () => {
            window.location.href = `/estudiantes/edit/${estudiante.id}`;
        });
...
window.onload = async function() {
    tbEstudiantes = document.getElementById("tbEstudiantes");
